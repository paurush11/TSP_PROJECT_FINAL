...
public class Ant implements Callable<Ant>{
	public static final double Q = 0.0005; // parameter used for adjusting the amount of pheromone deposited. the value is between 0 and 1
	public static final double RHO = 0.2; // parameter used for varying the level of phermone evaporation. the value is between 0 and 1
	public static final double ALPHA = 0.01; // parameter used for controlling the importance of the phermone trail. the value is >=0
	public static final double BETA = 9.5; // parameter used for controlling the importance of the distance between source and destination.
	private AntColonyOptimization aco;
	private int antNumb;
	private Route route = null;
	static int invalidCityIndex = -1;
	static int numbOfCities = Driver.graph.getNodes().size();
	...
	public Ant call() throws Exception {
		...
		ArrayList<Node> routeCities = new ArrayList<Node>(numbOfCities);
		HashMap<String, Boolean> visitedCities = new HashMap<String, Boolean>(numbOfCities);
		IntStream.range(0, numbOfCities).forEach(x -> visitedCities.put(Driver.graph.getNodes().get(x).getName(), false));
		int numbOfVisitedCities = 0;
		visitedCities.put(Driver.graph.getNodes().get(originatingCityIndex).getName(), true);
		...
		while(y != invalidCityIndex) {
			routeCities.add(numbOfVisitedCities++, Driver.graph.getNodes().get(x));
			routeDistance += aco.getDistancesMatrix()[x][y];
			adjustPhermoneLevel(x, y, routeDistance);
			visitedCities.put(Driver.graph.getNodes().get(y).getName(), true);
			x = y;
			if (numbOfVisitedCities != numbOfCities) y = getY(x, visitedCities);
	...
	private double getTPDenominator(ArrayList<Double> transitionProbabilities, int x, HashMap<String, Boolean> visitedCities) {
		...
		for (int y = 0; y < numbOfCities; y++) {
			if (!visitedCities.get(Driver.graph.getNodes().get(y).getName())) {
